...
namespace QLCafe.DAL
{
    public class BillDetailReponsitory : BaseRepository, IBillDetailReponsitory
    {
        ...
        public int BillDetailDelete(int Id)
        {
            try
            {
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("@Id", Id);
                var result = SqlMapper.ExecuteScalar<int>(con, "BillDetails_Delete", param: parameters, commandType: CommandType.StoredProcedure);
                return result;
            }
            catch (Exception ex)
            {
